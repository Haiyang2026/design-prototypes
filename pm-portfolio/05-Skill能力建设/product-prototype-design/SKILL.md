...
description: 快速生成高保真的产品交互原型（单个独立 HTML
  文件），适用于产品经理、创业者、设计师快速验证想法。当用户提到原型设计、产品原型、交互原型、demo 制作、产品 Mock、界面原型、H5
  原型、产品页面设计、MVP 原型、 Landing Page、产品概念验证，或需要快速将产品想法可视化为可交互页面时，请使用此
  skill。即使未明确提及'原型'，只要用户想快速做一个产品页面/界面/Demo，也应触发此 skill。
disable: true
---
...
除了标准原型结构外，还支持以下预置布局模式。当用户需求匹配时，优先选用。

| 模式 | 说明 | 适用场景 | 参考文档 |
|------|------|----------|----------|
| **弹性面板布局** | 左图右文，Web端默认紧凑折叠，可展开 | 功能介绍、流程说明、分支模拟 | `references/flexible-panel-layout.md` |
| **二次进入模拟** | 一排状态标签按钮，模拟不同用户路径 | 产品原型演示、测试用例说明 | `references/flexible-panel-layout.md` |

> **弹性面板核心规则**：Web 端所有内容区块**默认折叠**，首个区块展开。右侧内容可包含步骤流、操作区、状态分流表等子模块。手风琴模式（展开新区块时自动收起其他）。
